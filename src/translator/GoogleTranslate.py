...
class GoogleTranslate(Translatable):
    ...
    def translate(self, text: str) -> str:
        ...
        # src_text = self.browser.run_and_wait(100, find_src_box)
        # src_text.clear()
        ...
        # dst_text = self.browser.run_and_wait(100, find_trans_box)
        # time.sleep(1)
        # # dst_text = driver.find_element_by_xpath('//div[@class="text-wrap tlid-copy-target"]')
        # prev_text = dst_text.text
        translator = Translator()
        try:
            trans = translator.translate(text=text, src=self.source_langid, dest=self.target_langid)
        except:
            trans=''
        return trans
